...
export default function PersistentTaskForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Task created and added to To Do!');
      resetForm();
    } catch {
    ...
    <section className="bg-card border border-border rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-border bg-muted/30">
        <div className="w-8 h-8 rounded-xl bg-primary/15 flex items-center justify-center">
          <ClipboardPlus size={16} className="text-primary" />
        </div>
        <div>
          <h2 className="text-sm font-semibold text-foreground">Add New Task</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2.5 bg-muted border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all ${
                showTitleError ? 'border-red-500/60 focus:ring-red-500/30' : 'border-border focus:border-primary/50'
              }`}
            />
            {showTitleError && (
              <p className="text-xs text-red-400 mt-1">Task title is required</p>
            )}
          </div>
